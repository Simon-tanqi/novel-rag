...
背景（2026-09）：
用户问「主角叫什么名字？」时系统答不上来。排查发现不是检索链路故障
（RRF 融合 + CUDA 向量检索均正常），而是查询串本身只含「元词」——
「主角」是一个指代性词汇，全书第三人称直呼主角名（如本节示例书的主角
「陈曦」），书首登场片段与「主角叫什么名字」的语义相似度低，排不进
top_k；召回内容全是无关配角对白，最终触发 Prompt 的「无相关信息」规则
拒答。
